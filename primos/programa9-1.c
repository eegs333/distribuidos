...
int main(void)
{
   ...
   while(1) {
      ...
      inet_ntop(AF_INET, &(msg_to_client_addr.sin_addr), buffer, len);
      printf("address:%s\n",buffer);
      printf("port:%d\n", (int)ntohs(msg_to_client_addr.sin_port));
      res = num[0] + num[1];

      /* envía la petición al cliente. La estructura msg_to_client_addr contiene la dirección socket del cliente */
      sendto(s, (char *)&numero, sizeof(int), 0, (struct sockaddr *)&msg_to_client_addr, clilen);
   }
}
